...
def chat():
    ...
    if step == "ask_otp":
        ...
        if message == correct_otp:
            ...
            return jsonify({
                "reply": f"Number verified successfully.\nChoose doctor:\n{doctor_text}"
            })
    ...
    if step == "ask_doctor":
        ...
        if message not in valid_doctor_ids:
            return jsonify({"reply": "Please choose a valid doctor number."})
        ...
        return jsonify({
            "reply": "Enter appointment date in this format:\nYYYY-MM-DD\nExample: 2026-06-10"
        })
    ...
    if step == "ask_date":
        ...
        return jsonify({
            "reply": "Enter appointment time in this format:\nHH:MM\nExample: 10:30"
        })

    if step == "ask_time":
        if not is_valid_time(message):
            return jsonify({
                "reply": "Invalid time format.\nPlease enter time like this:\nHH:MM\nExample: 10:30"
            })
        ...
        if doctor_row.empty:
            return jsonify({"reply": "Doctor not found. Please start again."})
    ...
    if step == "completed":
        if message == "1":
            ...
            return jsonify({
                "reply": "Starting a new appointment booking.\nPlease enter your name."
            })

        if message == "2":
            return jsonify({
                "reply": "Reception will contact you soon. Thank you."
            })

        return jsonify({
            "reply": "You already have a completed booking.\n\nType:\n1. Book new appointment\n2. Contact reception"
        })
